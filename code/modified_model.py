...
def itemcf_recall(topk=10):
    ...
    def get_past_click():
        ...
        ###                    注释要去掉↓
        all_click_df = train_past_clicks._append(test_click)
...
def hot_recall(topk=10, train_past_clicks=None, test_last_click=None):
    ...
    test_hot_articles = pd.DataFrame(
        test_click_df["click_article_id"].value_counts().index.to_list(),
        columns=["article_id"],
    )
    ...
    for user_id in tqdm(train_past_clicks["user_id"].unique()):
        ...
        item_topk_click = get_item_topk_click_(
            train_hot_articles,
            train_hot_articles_dict,
            click_time,
            past_click_articles,
            k=topk,
        )
...
def get_test_recall(itemcf=False, hot=False):
    ...
    if itemcf:
        ...
        if hot:
            test = test._append(hot_test_recall)
...
def get_train_recall(itemcf=False, hot=False, train_last_click=None):
    ...
    if hot:
        ...
        hot_train_recall["label"] = hot_train_recall.merge(train_last_click, on=["user_id", "article_id"], how="left")[
            "click_timestamp"
        ].apply(lambda x: 0.0 if np.isnan(x) else 1.0)
        print(
            "Train Hot RECALL:{}%".format(
...
def train_and_predict(itemcf=False, itemcf_topk=10, hot=False, hot_topk=10, offline=True):
    ...
    if itemcf:
        # 使用用户历史点击记录去进行召回，判断召回中的文章是否有最后一次点击的文章，从而计算召回率
