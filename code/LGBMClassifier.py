...
if __name__ == "__main__":
    ...
    # 模型及参数的定义
    lgb_Classfication = lgb.LGBMClassifier(
        boosting_type="gbdt",
        num_leaves=31,
        reg_alpha=0.0,
        reg_lambda=1,
    ...
    tst_user_item_feats_df[["user_id", "click_article_id", "pred_score"]].to_csv(
        save_path + "/lgb_cls_score.csv", index=False
    )
    ...
    for n_fold, valid_user in enumerate(user_set):
        ...
        # 预测验证集结果
        valid_idx["pred_score"] = lgb_Classfication.predict_proba(
            valid_idx[lgb_cols], num_iteration=lgb_Classfication.best_iteration_
        )[:, 1]
        ...
        if not offline:
            sub_preds += lgb_Classfication.predict_proba(
                tst_user_item_feats_df_rank_model[lgb_cols], num_iteration=lgb_Classfication.best_iteration_
            )[:, 1]
