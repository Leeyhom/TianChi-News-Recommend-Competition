...
# 读取召回列表
# 返回多路召回列表或者单路召回
def get_recall_list(multi_recall_path, single_recall_model=None, multi_recall=False):
    if multi_recall:
        return pickle.load(open(multi_recall_path + "/final_recall_items_dict.pkl", "rb"))

    if single_recall_model == "i2i_itemcf":
        return pickle.load(open(multi_recall_path + "/itemcf_recall_dict.pkl", "rb"))
    elif single_recall_model == "i2i_emb_itemcf":
        return pickle.load(open(multi_recall_path + "/itemcf_emb_dict.pkl", "rb"))
    elif single_recall_model == "user_cf":
        return pickle.load(open(multi_recall_path + "/youtubednn_usercf_dict.pkl", "rb"))
    elif single_recall_model == "youtubednn":
        return pickle.load(open(multi_recall_path + "/youtube_u2i_dict.pkl", "rb"))


# ------------------------------各种embedding
def train_item_word2vec(click_df, embed_size=64, sace_name="item_w2v_emb.pkl", spilt_char=" "):
    ...
    pickle.dump(item_w2v_emb_dict, open(multi_recall_path + "/item_w2v_emb.pkl", "wb"))

    return item_w2v_emb_dict


# 可以通过字典查询对应的item的Embedding
def get_embedding(multi_recall_path, all_click_df):
    if os.path.exists(multi_recall_path + "/item_content_emb.pkl"):
        item_content_emb_dict = pickle.load(open(multi_recall_path + "/item_content_emb.pkl", "rb"))
    else:
        print("item_content_emb.pkl 文件不存在...")

    # w2v Embedding是需要提前训练好的
    if os.path.exists(multi_recall_path + "/item_w2v_emb.pkl"):
        item_w2v_emb_dict = pickle.load(open(multi_recall_path + "/item_w2v_emb.pkl", "rb"))
    else:
        item_w2v_emb_dict = train_item_word2vec(all_click_df)

    if os.path.exists(multi_recall_path + "/item_youtube_emb.pkl"):
        item_youtube_emb_dict = pickle.load(open(multi_recall_path + "/item_youtube_emb.pkl", "rb"))
    else:
        print("item_youtube_emb.pkl 文件不存在...")

    if os.path.exists(multi_recall_path + "/user_youtube_emb.pkl"):
        user_youtube_emb_dict = pickle.load(open(multi_recall_path + "/user_youtube_emb.pkl", "rb"))
    else:
...
def neg_sample_recall_data(recall_items_df, sample_rate=0.001):
    ...
    # 将上述两种情况下的采样数据合并
    neg_data_new = neg_data_user_sample._append(neg_data_item_sample)
    ...
    for user_id in tqdm(users_id):
        ...
        for rank, (article_id, score, label) in enumerate(recall_list[user_id]):
            ...
            single_user_fea.extend(
                [max(sim_fea), min(sim_fea), sum(sim_fea), sum(sim_fea) / len(sim_fea)]
            )  # 相似性的统计特征
...
def active_level(all_data, cols):
    ...
    user_act = pd.DataFrame(
        ...
        .agg({"click_article_id": np.size, "click_timestamp": {list}})
        .values,
...
def hot_level(all_data, cols):
    ...
    article_hot = pd.DataFrame(
        ...
        .agg({"user_id": np.size, "click_timestamp": {list}})
        .values,
...
def user_time_hob_fea(all_data, cols):
    ...
    user_time_hob_info.rename(
        columns={"click_timestamp": "user_time_hob1", "created_at_ts": "user_time_hob2"}, inplace=True
    )
...
if __name__ == "__main__":
    ...
    if not test:
        ...
        recall_list_dict = get_recall_list(
            multi_recall_path, single_recall_model="i2i_itemcf"
        ...
        trn_user_item_label_tuples_dict = dict(
            zip(trn_user_item_label_tuples["user_id"], trn_user_item_label_tuples[0])
        )
        ...
        tst_user_item_label_tuples_dict = dict(
            zip(tst_user_item_label_tuples["user_id"], tst_user_item_label_tuples[0])
        )

    # 制作特征工程--------------------------------
    article_info_df = get_article_info_df()
    all_click = click_trn._append(click_tst)
    # 获取embedding向量
    (
        item_content_emb_dict,
        item_w2v_emb_dict,
        item_youtube_emb_dict,
        user_youtube_emb_dict,
    ) = get_embedding(multi_recall_path, all_click)

    # 获取训练验证及测试数据中召回列文章相关特征
    # 如果文件不存在，就重新跑一遍
    if not os.path.exists(save_path + "/trn_user_item_feats_df.csv"):
